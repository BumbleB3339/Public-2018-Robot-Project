...
public class PixyI2C {
	...
	/*
	 * public void pixyReset(){ pixy.reset(); }
	 */
	...
	public PixyPacket readPacket(int Signature) throws PixyException { // The signature should be which number object in
		...
		for (int i = 0; i <= 16; i++) {
			...
			if (syncWord == 0xaa55) { // Check is first 2 bytes equal a "sync word", which indicates the start of a
		...
		// Assigns our packet to a temp packet, then deletes data so that we dont return
		// old data
