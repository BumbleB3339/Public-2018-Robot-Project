...
public class PixySPI {
	...
	public int readPackets() throws PixyException { // The signature should be which number object in
		...
		// Uncomment this to see just the raw output from the Pixy. You will need to
		...
		// Put the found blocks into the correct spot in the return
		// Hashmap<ArrayList<int[]>>.
		if (numBlocks > 0) {
			if (debug >= 2) {
			...
			for (int i = 0; i < numBlocks; i++) {
				...
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: Signature: " + Integer.toString(signature),
							Integer.toString(signature));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": X: ",
							Integer.toString(packet.X));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Y: ",
							Integer.toString(packet.Y));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Width: ",
							Integer.toString(packet.Width));
				}
				if (debug >= 1) {
					SmartDashboard.putString("Pixy readPackets: " + Integer.toString(signature) + ": Height: ",
							Integer.toString(packet.Height));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: Signature: " + Integer.toString(signature),
							Integer.toString(signature));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": X: ",
							Integer.toString(packet.X));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Y: ",
							Integer.toString(packet.Y));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Width: ",
							Integer.toString(packet.Width));
				}
				if (debug >= 2) {
					logger.log(Level.INFO, "Pixy readPackets: " + Integer.toString(signature) + ": Height: ",
							Integer.toString(packet.Height));
				}
	...
	private int getBlocks(int maxBlocks) {
		...
		// Loop until we hit the maximum size allowed for blocks, or until we know we
		// have a complete set of blocks.
		while ((blocks.size()) < maxBlocks && (blocks.size() < PIXY_MAXIMUM_ARRAYSIZE)) {
			if (debug >= 1) {
				SmartDashboard.putNumber("getBlocks: loop count: ", count++);
			}

			// Since this is our first time in, bytes 2 and 3 are the checksum, grab them
			...
			// to skip looking for the beginning of the next block since we already found
			// it.
			...
			// See if we received a empty buffer, if so, assume end of comms for now and
			// return what we have.
			else if (checksum == 0) {
				if (debug >= 2) {
			...
			// Only need 5 slots since the first 3 slots, the double start blocks and
			// checksum, have been retrieved already.
			int[] block = new int[5];
			...
			if (checksum == trialsum) {
				// Data has been validated, add the current block of data to the overall blocks
				// buffer.
				blocks.add(block);
				if (debug >= 2) {
			...
			// Check the next word from the Pixy to confirm it's the start of the next
			// block.
	...
	private int getWord() {
		...
		// Store the value of the buffer to a string for debugging to the console/log
		// and SmartDashboard.
		if (debug >= 1) {
		...
		// Set the position back to 0 in the buffer so we read it from the beginning
		// next time.
		readBuf.rewind();

		// Store the contents of the buffer in a int that will be returned to the
		// caller.
	...
	private boolean getStart() {
		...
		while (true) {
			...
			if ((w == 0x00) && (lastw == 0x00)) {
				// Could delay a bit to give time for next data block, but to get accurate time
				// would tie up cpu.
